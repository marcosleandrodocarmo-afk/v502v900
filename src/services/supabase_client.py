...
class SupabaseClient:
    ...
    def create_analysis(self, analysis_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # CORREÇÃO: Tenta criar mesmo se validação falhar (pode ser problema de migração)
        try:
            if not self._validate_api_key():
                logger.warning("⚠️ Validação de API falhou, tentando criar mesmo assim...")
        ...
        try:
            ...
            # CORREÇÃO: Log mais detalhado e não falha o sistema
            error_msg = str(e)
            if 'relation "analyses" does not exist' in error_msg:
                logger.error("❌ Tabela 'analyses' não existe. Execute as migrações do Supabase primeiro.")
            elif 'invalid api key' in error_msg.lower():
                logger.error("❌ Chave de API do Supabase inválida. Verifique SUPABASE_ANON_KEY no .env")
            else:
                logger.error(f"❌ Erro ao criar análise no Supabase: {error_msg}")
            return None
    ...
    def _validate_api_key(self) -> bool:
        ...
        try:
            ...
            if 'invalid api key' in error_str or 'unauthorized' in error_str:
                ...
                return False
            elif 'relation "analyses" does not exist' in error_str:
                logger.warning("⚠️ Tabela 'analyses' não existe - execute as migrações do Supabase")
                return False
            # Outros erros podem ser temporários
            logger.warning(f"⚠️ Erro na validação da API: {e}")
            return False
