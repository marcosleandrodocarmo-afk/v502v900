...
class ProductionSearchManager:
    ...
    def _search_bing(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        ...
        # CORREÇÃO: Adiciona tratamento de timeout e SSL
        ...
        try:
            response = requests.get(
                search_url, 
                headers=self.headers, 
                timeout=10,  # CORREÇÃO: Timeout menor
                verify=False  # CORREÇÃO: Desabilita SSL
            )
        except requests.exceptions.SSLError:
            logger.warning(f"⚠️ Erro SSL no Bing para query: {query}")
            return []
        except requests.exceptions.Timeout:
            logger.warning(f"⚠️ Timeout no Bing para query: {query}")
            return []
        except Exception as e:
            ...
            for item in result_items[:max_results]:
                ...
                if title_elem:
                    ...
                    if link_elem:
                        ...
                        # CORREÇÃO: Filtra URLs problemáticas
                        if url and title and url.startswith('http') and not self._is_problematic_url(url):
                            results.append({
                                'title': title,
                                'url': url,
                                'snippet': snippet,
    ...
    def _search_duckduckgo(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        ...
        # CORREÇÃO: Adiciona tratamento de erros
        ...
        try:
            ...
            logger.warning(f"⚠️ Erro no DuckDuckGo: {e}")
            return []
