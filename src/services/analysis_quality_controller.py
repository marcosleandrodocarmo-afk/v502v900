...
class AnalysisQualityController:
    ...
    def __init__(self):
        ...
        self.quality_thresholds = {
            'min_content_length': 200,  # Ainda mais flexível
            'min_sources': 1,           # Muito mais flexível
            'min_insights': 1,          # Muito mais flexível
            'min_quality_score': 20.0,  # Muito mais flexível
            'min_component_success_rate': 0.2  # 20% mínimo
        }
        
        self.simulation_indicators = [
            'n/a explícito',  # Apenas N/A explícito
            'dados simulados',
            'exemplo genérico'
        ]
    ...
    def _validate_structure(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for section in required_sections:
            if section not in analysis:
                # CORREÇÃO: Aviso em vez de erro crítico
                result['warnings'].append(f"Seção recomendada ausente: {section}")
            elif not analysis[section]:
                result['warnings'].append(f"Seção recomendada vazia: {section}")
        
        # CORREÇÃO: Só falha se TODAS as seções estão ausentes
        present_sections = [s for s in required_sections if s in analysis and analysis[s]]
        if len(present_sections) == 0:
            result['errors'].append("Nenhuma seção obrigatória presente")
            result['valid'] = False
    ...
    def _validate_research_data(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not research_data:
            # CORREÇÃO: Aviso em vez de erro
            result['warnings'].append("Dados de pesquisa web ausentes")
            return result
        ...
        if total_conteudo < self.quality_thresholds['min_content_length']:
            result['warnings'].append(f"Conteúdo limitado: {total_conteudo} < {self.quality_thresholds['min_content_length']}")
        
        fontes_unicas = estatisticas.get('fontes_unicas', 0)
        if fontes_unicas < self.quality_thresholds['min_sources']:
            result['warnings'].append(f"Fontes limitadas: {fontes_unicas} < {self.quality_thresholds['min_sources']}")
    ...
    def _validate_avatar(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not avatar:
            result['warnings'].append("Avatar ausente")
            return result
        ...
        missing_avatar_sections = 0
        for section in required_avatar_sections:
            if section not in avatar or not avatar[section]:
                result['warnings'].append(f"Seção do avatar ausente: {section}")
                missing_avatar_sections += 1
        
        # CORREÇÃO: Só falha se mais de 50% das seções estão ausentes
        if missing_avatar_sections > len(required_avatar_sections) / 2:
            result['errors'].append("Avatar muito incompleto")
            result['valid'] = False
        
        # Valida listas mínimas
        dores = avatar.get('dores_viscerais', [])
        if len(dores) < 3:  # CORREÇÃO: Mais flexível
            result['warnings'].append(f"Poucas dores: {len(dores)} < 3")
        
        desejos = avatar.get('desejos_secretos', [])
        if len(desejos) < 3:  # CORREÇÃO: Mais flexível
            result['warnings'].append(f"Poucos desejos: {len(desejos)} < 3")
    ...
    def _detect_simulated_content(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result['simulation_count'] > 100:  # CORREÇÃO: Tolerância ainda maior
    ...
    def _check_field_for_simulation(self, analysis: Dict[str, Any], field_name: str, result: Dict[str, Any]):
        ...
        simulation_patterns = [
            'dados claramente simulados',  # CORREÇÃO: Apenas padrões óbvios
            'exemplo fictício',
            'placeholder text'
        ]
